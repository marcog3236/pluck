...
export function TrumpSelector({ onSelect }: TrumpSelectorProps) {
  return (
    <motion.div
      className="flex flex-col items-center gap-3 sm:gap-4 bg-slate-800/90 backdrop-blur rounded-2xl p-4 sm:p-6 border border-slate-600 shadow-2xl mx-4"
      initial={{ scale: 0.9, opacity: 0 }}
      animate={{ scale: 1, opacity: 1 }}
      transition={{ type: "spring", stiffness: 300, damping: 25 }}
    >
      <h3 className="text-base sm:text-lg font-bold text-slate-100">Choose Trump</h3>
      <p className="text-xs sm:text-sm text-slate-400">Pick your strongest suit</p>
      <div className="flex gap-2 sm:gap-3">
        {SUITS.map(({ suit, symbol, color, label }) => (
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
          >
            <span className={`text-3xl sm:text-4xl ${color}`}>{symbol}</span>
